  ...
  useEffect(() => {
    async function fetchAllostaticData() {
      ...
      const { data, error } = await supabase
        .from('daily_logs')
        .select('log_date, stress_level, simulated_rmssd, fatigue_level')
        .eq('user_id', user.id)
        .gte('log_date', ninetyDaysAgo.toISOString().split('T')[0])
        .order('log_date', { ascending: true })

      if (!error && data) {
        const transformed = (data as ProgressLog[]).map(log => ({
          date: new Date(log.log_date).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' }),
          estres: log.stress_level,
          recuperacion: log.simulated_rmssd,
          burnout: (((log.stress_level ?? 0) + (log.fatigue_level || 0)) / 2)
        }))
